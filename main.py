...
def build_device( mask_obj, cell_name, area):

    mask_f.build_mesas( mask_obj, cell_name, area)
    mask_f.build_WF_markers( mask_obj, cell_name)
    mask_f.build_pads( mask_obj, cell_name)
    ...
    mask_f.SiN_opening( mask_obj, cell_name, area)
    # build_shunting( layout, cell_name, slot_l, area)
    mask_f.build_FE( mask_obj, cell_name)

def build_AUX( mask_obj, cell_name):
    mask_f.build_Global_markers( mask_obj, cell_name)
    mask_b.write_text(mask_obj.layout, cell_name, "FE_text", 0.000004, -600, 4000, 90, "MESA_TEST_N8")
    # mask_b.write_text(mask_obj.layout, cell_name, "FE_text", 0.000004, -600, 4000, 90, "MESA_TEST_N9")
    # mask_b.write_text(mask_obj.layout, cell_name, "FE_text", 0.000004, -600, 4000, 90, "MESA_TEST_N12")
    # mask_b.write_text(mask_obj.layout, cell_name, "FE_text", 0.000004, -600, 4000, 90, "MESA_TEST_N13")
...
def post_mortem(mask_obj, top_cell):
    ...
    path = os.path.join( "..", "gds", "mask_mesa_with_pads_1", "mesa_test_N8t.gds")
...
def main():
    ...
    for row_index in range(mask_instace.n_x_devices):
        for column_index in range(mask_instace.n_y_devices):
            ...
            build_device( mask_instace, active_cell_name, mask_instace.meas_areas[mesa_radius_index])
